...
async def toggle_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with storage.lock(f"chat-settings:{chat_id}") as acquired:
        if not acquired:
            ...
            return
        # Reload before mutating so two workers cannot overwrite each other.
        saved = await storage.load(STORAGE_KEY, {"enabled": {}, "persona": {}})
        ...
        chat_enabled.update(enabled)
        chat_persona.update(personas)
        await storage.save(STORAGE_KEY, {"enabled": enabled, "persona": personas})
...
async def set_persona(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = str(update.effective_chat.id)
    style = " ".join(context.args).strip() if context.args else DEFAULT_PERSONA
    style = style[:300]
    async with storage.lock(f"chat-settings:{chat_id}") as acquired:
        if not acquired:
            await update.message.reply_text("⏳ Chat settings are busy — try again.")
...
async def auto_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    was_keyword_triggered = "midnight" in message.text.lower()
...
async def generate_reply(user_text: str, persona: str, history: list) -> str | None:
    """Compatibility wrapper: all generation now goes through ``core.ai``."""
    try:
