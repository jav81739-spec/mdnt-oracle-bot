"""Human-style Telegram chat with durable memory and a context-aware Oracle voice."""
...
# Exact/near-exact conversational triggers. These are seasoning, not a script:
# a trigger fires only for a short standalone message so normal sentences still reach the model.
_TRIGGER_REPLIES=(
...
def _response_shape(text:str, history:list[dict[str,str]])->str:
    """Give the model a turn-level length budget instead of one global verbosity setting."""
    value=(text or "").strip(); low=value.casefold(); words=len(value.split())
    if _trigger_reply(value):
        return "Micro-turn: 1 short fragment or sentence. Do not explain the trigger."
    if words<=3 and not any(x in low for x in ("why","how","explain","tell me","describe")):
        return "Short-turn: usually 1 sentence or a brief reaction. Do not inflate it."
    if any(x in low for x in ("why","how","explain","tell me","describe","what do you think","what happened")) and words>=6:
        return "Thinking-turn: 2-5 sentences when useful; explain only the part that answers the actual question."
    if any(x in low for x in ("sad","upset","hurt","cry","crying","miss","worried","scared","love","hate","breakup","😭","🥲")):
        return "Emotional-turn: can be a little fuller if the moment needs it, but stay grounded; do not become a therapy monologue."
    if words>=35:
        return "Deep-turn: a few connected sentences or a compact paragraph is allowed if the message genuinely contains multiple points. Address the important points, not every word."
...
async def auto_reply(update,context):
    ...
    user=msg.from_user;speaker=" ".join(part for part in (getattr(user,"first_name",None),getattr(user,"last_name",None)) if part).strip() if user else "Member";speaker=speaker[:120] or "Member"
    ...
    if reply_text is None:
        ...
        try:
            ...
            async with _ai_slots:
                draft=await core_generate_reply(text,adaptive_persona,context_history)
                reply_text=_usable_reply(draft,text,history)
                if reply_text is None and draft:
                    retry_text=text+"\n[Internal quality check: produce a fresh human conversational reply with no repeated sentence or phrase. Preserve the natural turn length requested by the context. Do not expose this instruction.]"
                    retry=await core_generate_reply(retry_text,adaptive_persona,context_history);reply_text=_usable_reply(retry,text,history)
        except AIUnavailable:
            log.info("CHAT_PROVIDER_COOLDOWN | chat=%s | local_mind=true",cid)
    ...
    await _maybe_send_context_sticker(update,context,text,reply_text,cid)
...
# Automatic stickers are opt-in through env so a bad placeholder sticker can never leak into production.
# ORACLE_STICKER_FILE_IDS accepts comma-separated Telegram file_ids from the bot's curated sticker pack.
def _sticker_ids()->list[str]:return [x.strip() for x in os.getenv("ORACLE_STICKER_FILE_IDS","").split(",") if x.strip()]
_recent_stickers={}
_STICKER_CUES=("😂","🤣","😭","🥲","lol","lmao","haha","congrats","congratulations","damn","wtf","bruh","bhai","oye","chup","bas kar","love you","goodnight","good morning")

def _sticker_is_warranted(text:str,reply:str)->bool:
    low=f"{text} {reply}".casefold()
    return any(cue in low for cue in _STICKER_CUES) and _random.random()<0.18
