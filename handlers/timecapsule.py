...
async def timecapsule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if delay is None or delay > 365 * 86400:
        await update.message.reply_text("Invalid delay. Use a positive duration such as 10m, 2h, or 3d (max 365d).")
        return
...
async def load_and_reschedule(app: Application):
    ...
    for chat_id,pending in list(capsules.items()):
        for capsule in list(pending):
            ...
            app.job_queue.run_once(_fire_capsule, when=max(1,remaining), data=capsule, chat_id=int(chat_id), name=f"capsule:{capsule['id']}")
