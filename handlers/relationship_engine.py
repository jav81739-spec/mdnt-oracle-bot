...
def _resolve(update,context,members):
    ...
    if msg and msg.reply_to_message and msg.reply_to_message.from_user:
        rid=msg.reply_to_message.from_user.id; second=second or next((m for m in members if int(m.get("id",-1))==rid),None)
...
def _reading(kind,a,b,state):
    pairs=_pair_display(a,b); titles={"thread":"THE WEAVE","orbit":"ORBIT TRACE","echo":"ECHO FOUND","tether":"THE ANCHOR","rift":"THE FRACTURE","spark":"EMBER TRACE","mirror":"MIRROR READING","crossing":"CROSSING","undertow":"UNDERTOW","verdict":"ORACLE EDICT"}
...
async def _watch_change(update,context,enabled):
    ...
    if not chat:return
    if not a or not b: await update.effective_message.reply_text("☾ Use /gaze @member @member — or reply to a member.");return
...
async def sealed(update,context):
    chat,_,members=await _targets(update); a,b=_resolve(update,context,members) if chat else (None,None)
    if not chat:return
    if not a or not b: await update.effective_message.reply_text("☾ Use /veil @member @member — or reply to a member.");return
...
COMMANDS={"weave":"thread","orbit":"orbit","echo":"echo","anchor":"tether","fracture":"rift","ember":"spark","mirror":"mirror","crossing":"crossing","undertow":"undertow","edict":"verdict"}
