...
async def oracle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🔮 *The Oracle speaks on \"{question}\":*\n\n_{random.choice(ORACLE_LINES)}_"
    from handlers.chat import send_text_with_gif
    await send_text_with_gif(context.bot, update.effective_chat.id, text, "mystical crystal ball")


async def tarot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    card, meaning = random.choice(TAROT_CARDS)
    text = f"🃏 You drew: *{card}*\n\n_{meaning}_"
    from handlers.chat import send_text_with_gif
    await send_text_with_gif(context.bot, update.effective_chat.id, text, "tarot cards mystical")
