...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "*🎮 More Games*\n"
        "/hangman + /hangmanguess [letter]\n"
        "/tictactoe (reply) + /ttt [1-9]\n"
        "/wordchain + /chainword [word]\n"
        "/trivia [movies/cricket/general]\n"
        "/wordle + /wordleguess [word]\n\n"
        "*💰 Economy*\n"
        "/daily — claim daily coins\n"
        "/balance — check coins\n"
        "/rob (reply) /gamble [amount]\n"
        "/richest — coin leaderboard\n\n"
        "*🌙 More Aesthetic*\n"
        "/moodboard /dream [text] /manifest [text]\n\n"
        "*👥 More Social*\n"
        "/matchmaker — bot pairs two random members\n"
        "/friendshiptest (reply)\n"
        "/hug /pat /highfive /slap /kiss /poke /cuddle /wave /bite /tickle (reply)\n"
        "_(all now come with a matching gif!)_\n\n"
        "*🎉 More Fun*\n"
        "/ratethis (reply) — cheeky rating\n"
        "/impostor — start a round, /revealimpostor to end\n\n"
        "*⏳ Time Capsule*\n"
        "/timecapsule [delay] [message] — e.g. /timecapsule 2h see you later\n"
        "/capsules — list pending capsules\n\n"
        "*💍 Marriage & Shop*\n"
        "/marry (reply) — propose /accept — accept a proposal\n"
        "/divorce /profile (reply?)\n"
        ...
        "/deathgame — start a Mafia-style lobby\n"
        "/joingame /startround (host only)\n"
        "/kill (DM only, killer role) /vote [number]\n"
        "/endgame (host only)"
    )
