...
def register_legacy_surface(app):
    ...
    try:
        ...
        if callable(getattr(legacy_bot,"midnight_member_welcome",None)):app.add_handler(__import__('telegram.ext',fromlist=['ChatMemberHandler']).ChatMemberHandler(legacy_bot.midnight_member_welcome,__import__('telegram.ext',fromlist=['ChatMemberHandler']).ChatMemberHandler.CHAT_MEMBER))
