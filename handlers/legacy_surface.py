...
async def _is_owner(update):
    try:
        return int(os.getenv("OWNER_ID", "0") or "0") == int(update.effective_user.id)
    except Exception:
        return False
...
def _add_command(app, existing, command, target, callback_name, *, group=0):
    ...
    if cb is None:
        raise RuntimeError(f"COMMAND_CALLBACK_MISSING: /{name} -> {getattr(target, '__name__', target)}.{callback_name}")
    app.add_handler(CommandHandler(name, _protected_callback(cb, name)), group=group)
    existing.add(name)
    return True
...
def _assert_no_duplicate_declarations(module_commands, direct):
    ...
    for module, commands in module_commands.items():
        for command in commands:
            previous = owners.setdefault(command, module)
            if previous != module:
                raise RuntimeError(f"COMMAND_OWNER_COLLISION: /{command}: {previous} vs {module}")
    for command in direct:
        if command in owners:
            raise RuntimeError(f"COMMAND_OWNER_COLLISION: /{command}: {owners[command]} vs legacy_bot")
    ...
    for mn, commands in module_commands.items():
        for command, cb in commands.items():
            if command in reserved or command in existing:
                continue
            if _add_command(app, existing, command, module_map[mn], cb):
                added.append(command)
    for command, cb in direct.items():
        if command in reserved or command in existing:
            continue
        if _add_command(app, existing, command, legacy_bot, cb):
            added.append(command)
    try:
