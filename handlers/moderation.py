...
async def mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /mute"); return
    try:
...
async def unmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /unmute"); return
    try:
...
async def ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /ban"); return
    try:
...
async def kick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /kick"); return
    try:
        await context.bot.ban_chat_member(update.effective_chat.id, target.id)
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /warn"); return
    ...
    async with storage.lock(f"warning:{chat_id}:{target.id}") as acquired:
        if not acquired: await update.message.reply_text("⏳ Moderation is busy — try again."); return
        raw = await storage.get(key, "0")
        try: count = int(raw or 0) + 1
        except (TypeError, ValueError): count = 1
        if not await storage.set(key, str(count)):
            await update.message.reply_text("⚠️ Warning could not be saved; no action was taken."); return
...
async def show_rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rules = await storage.get(_rules_key(update.effective_chat.id), "") or "No rules set yet. Admins can use /setrules to add them."
...
async def clear_warnings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    target = await _get_target(update)
    if not target: await update.message.reply_text("Reply to the user's message with /clearwarns"); return
...
async def pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    if not update.message.reply_to_message: await update.message.reply_text("Reply to the message you want to pin with /pin"); return
    try:
...
async def purge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    if not update.message.reply_to_message or not context.args: await update.message.reply_text("Reply to a message with /purge [1-100]"); return
    try: count = int(context.args[0])
    except (TypeError, ValueError): await update.message.reply_text("Usage: /purge [1-100]"); return
    if not 1 <= count <= 100: await update.message.reply_text("Choose a purge count between 1 and 100."); return
    ...
    if context.job_queue:
        context.job_queue.run_once(_delete_later, when=5, chat_id=chat_id, data=confirmation.message_id)


async def set_rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    if not context.args: await update.message.reply_text("Usage: /setrules [your rules text]"); return
    if not await storage.set(_rules_key(update.effective_chat.id), " ".join(context.args)[:4000]):
        await update.message.reply_text("⚠️ Rules could not be saved."); return
...
async def lock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lock_type == "media":
        permissions = ChatPermissions(can_send_messages=True, can_send_photos=False, can_send_videos=False, can_send_other_messages=False)
    elif lock_type == "all":
        permissions = ChatPermissions(can_send_messages=False)
    else:
        await update.message.reply_text("Usage: /lock media | /lock all"); return
    try:
...
async def unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _is_admin(update, context): await update.message.reply_text("Admins only."); return
    try:
        await context.bot.set_chat_permissions(update.effective_chat.id, ChatPermissions(can_send_messages=True, can_send_photos=True, can_send_videos=True, can_send_other_messages=True))
