...
import sys, os, logging, random, asyncio, json, hashlib, re, threading, html
...
# ══════════════════════════════════════════════════════════════════════════
# MIDNIGHT CONNECTION READING — persistent hidden cycle
# Publicly this is simply a reading. The internal clock stays invisible.
# ══════════════════════════════════════════════════════════════════════════

_CONNECTION_READINGS = [
    "There is a quiet pull here. It may be subtle, but it is not imaginary.",
    "Interesting connection. One side feels louder than the other right now.",
    "The signal is warm, but the timing is still uncertain.",
    "Something about this pairing feels unfinished rather than final.",
    "The chemistry is there. Whether it becomes anything else is another story.",
    "This connection carries more curiosity than certainty. Watch what happens next.",
    "A strange balance: easy to notice, harder to define.",
    "The energy is soft here. Not loud. Not accidental either.",
    "There is potential, but the next move matters more than the feeling.",
    "The connection is stronger in possibility than in proof. Keep that distinction.",
    "Something keeps pulling these two into the same orbit.",
    "The signal is mixed. That usually means the story is still being written.",
]

def _oracle_cycle_start(now=None):
    now = now or datetime.now(ORACLE_TZ)
    if now.tzinfo is None:
        now = now.replace(tzinfo=ORACLE_TZ)
    base = datetime(2020, 1, 1, ORACLE_CYCLE_START_H, ORACLE_CYCLE_START_M, tzinfo=ORACLE_TZ)
    if now < base:
        return base
    elapsed = now - base
    slots = int(elapsed.total_seconds() // (ORACLE_CYCLE_HOURS * 3600))
    return base + timedelta(hours=slots * ORACLE_CYCLE_HOURS)

def _oracle_cycle_id(now=None):
    return _oracle_cycle_start(now).strftime("%Y%m%d%H%M")

def _seconds_until_next_oracle_cycle(now=None):
    now = now or datetime.now(ORACLE_TZ)
    start = _oracle_cycle_start(now)
    end = start + timedelta(hours=ORACLE_CYCLE_HOURS)
    return max(60, int((end - now).total_seconds()))

def _connection_key(uid_a, uid_b):
    a, b = sorted((int(uid_a), int(uid_b)))
    return f"connection:{a}:{b}"

def _connection_reading(uid_a, uid_b, now=None):
    cycle = _oracle_cycle_id(now)
    a, b = sorted((int(uid_a), int(uid_b)))
    seed = hashlib.sha256(f"{a}:{b}:{cycle}".encode()).hexdigest()
    return _CONNECTION_READINGS[int(seed[:12], 16) % len(_CONNECTION_READINGS)]

async def bond_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """A quiet, persistent connection reading for two members."""
    message = update.message
    u = update.effective_user
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        ...
        try:
            target = await context.bot.get_chat(f"@{handle}")
        except Exception:
            target = None

    if not target:
        await message.reply_text("✦ Reply to someone — I'll read the connection.")
        return

    if target.id == u.id or target.is_bot:
        await message.reply_text("🌙 Some doors are better left unopened.")
        return

    reading = _connection_reading(u.id, target.id)
    key = _connection_key(u.id, target.id)
    await _rsetex(
        f"{key}:cycle",
        ORACLE_CYCLE_HOURS * 3600 + 7200,
        json.dumps({
            "cycle": _oracle_cycle_id(),
            "reading": reading,
            "a": u.id,
            "b": target.id,
        }, ensure_ascii=False),
    ...
    await message.reply_text(
        f"✦ *THE CONNECTION*\n\n"
        f"*{u.first_name}*  ×  *{target.first_name}*\n\n"
        f"_{reading}_\n\n"
...
# ══════════════════════════════════════════════════════════════════════════
# MIDNIGHT BONDS — automatic 24h bonds + rotating signal
# No join/start commands. Midnight chooses automatically.
# ══════════════════════════════════════════════════════════════════════════
#
# bond:{chat_id}              -> current fixed 24h cycle JSON
# signal:{chat_id}            -> current rotating signal JSON
# bond_history:{chat_id}     -> last few pair fingerprints
# oracle_active_members:*     -> recent active members persisted across restarts
#
# The daily bond is anchored to ORACLE_CYCLE_START_H/M (default 06:30 IST),
# not to bot restart time. This makes the 24h window genuinely fixed.

_BOND_ACTIVITY_DAYS = max(1, min(14, int(os.getenv("BOND_ACTIVITY_DAYS", "7"))))
_SIGNAL_HOURS = max(1, min(12, int(os.getenv("SIGNAL_HOURS", "3"))))
_BOND_MIN_MEMBERS = 2

async def _bond_active_members(chat_id):
    """Return recently active human members, surviving Render restarts."""
    raw = await _rget(f"oracle_active_members:{chat_id}")
    data = {}
    if raw:
        try:
            data = json.loads(raw)
            ...
            data = {}
    cutoff = int(datetime.now(ORACLE_TZ).timestamp()) - (_BOND_ACTIVITY_DAYS * 86400)
    changed = False
    for uid in list(data):
        try:
            if int(data[uid].get("last_seen", 0)) < cutoff:
                del data[uid]
                changed = True
        except Exception:
            del data[uid]
    ...
    for uid, info in data.items():
        try:
            if int(info.get("last_seen", 0)) >= cutoff:
                members.append({"uid": int(uid), "name": info.get("name") or "Member", "last_seen": int(info.get("last_seen", 0))})
        except Exception:
            continue
    members.sort(key=lambda x: x["last_seen"], reverse=True)
    if changed:
        await _rsetex(f"oracle_active_members:{chat_id}", (_BOND_ACTIVITY_DAYS + 2) * 86400, json.dumps(data, ensure_ascii=False))
    return members[:100]


def _bond_cycle_bounds(now=None):
    now = now or datetime.now(ORACLE_TZ)
    start = now.replace(hour=ORACLE_CYCLE_START_H, minute=ORACLE_CYCLE_START_M, second=0, microsecond=0)
    if now < start:
...
def _signal_cycle_bounds(now=None):
    now = now or datetime.now(ORACLE_TZ)
    anchor = datetime(1970, 1, 1, tzinfo=ORACLE_TZ)
    seconds = int((now - anchor).total_seconds())
    size = _SIGNAL_HOURS * 3600
    bucket = seconds // size
    start = anchor + timedelta(seconds=bucket * size)
    return start, start + timedelta(seconds=size)


def _remaining_text(end):
    total = max(0, int((end - datetime.now(ORACLE_TZ)).total_seconds()))
...
def _pair_fingerprint(a, b):
    x, y = sorted((int(a), int(b)))
    return f"{x}-{y}"


def _choose_pairs(members, previous=None):
    """Fair-ish shuffle while trying hard not to repeat yesterday's partner."""
    ...
    for _ in range(80):
        ...
        for i in range(0, len(pool) - 1, 2):
            a, b = pool[i], pool[i + 1]
            if previous.get(str(a["uid"])) == int(b["uid"]):
                repeats += 1
            if previous.get(str(b["uid"])) == int(a["uid"]):
                repeats += 1
            pairs.append((a, b))
        if repeats < best_repeat:
            best, best_repeat = pairs, repeats
            if repeats == 0:
                break
    return best or []


async def _make_bond(chat_id, start, end, announce=True, bot=None):
    members = await _bond_active_members(chat_id)
    if len(members) < _BOND_MIN_MEMBERS:
        return False
    ...
    if old_raw:
        try:
            ...
            previous = {str(k): int(v) for k, v in old.get("partners", {}).items()}
        except Exception:
            pass

    pairs = _choose_pairs(members, previous)
    if not pairs:
        return False
    ...
    partners = {}
    for a, b in pairs:
        partners[str(a["uid"])] = int(b["uid"])
        partners[str(b["uid"])] = int(a["uid"])
        pair_rows.append({"a": int(a["uid"]), "an": a["name"], "b": int(b["uid"]), "bn": b["name"]})

    data = {
        "kind": "bond", "start": start.isoformat(), "end": end.isoformat(),
        "pairs": pair_rows, "partners": partners,
        "created_at": int(datetime.now(ORACLE_TZ).timestamp())
    }
    await _rsetex(f"bond:{chat_id}", 3 * 86400, json.dumps(data, ensure_ascii=False))

    if bot and announce:
        names = [f"<b>{html.escape(x['an'])}</b> × <b>{html.escape(x['bn'])}</b>" for x in pair_rows]
        text = (
            "✦ <b>MIDNIGHT BOND</b>\n\n"
            "The new cycle is in.\n"
            "The names stay hidden until the veil lifts.\n\n"
            f"⏳ <b>{_remaining_text(end)}</b> remaining"
        )
        try:
            await bot.send_message(chat_id, text, parse_mode="HTML")
        except Exception as e:
            logger.info("Bond announcement skipped for %s: %s", chat_id, e)
    return True


async def _make_signal(chat_id, start, end, announce=True, bot=None):
    members = await _bond_active_members(chat_id)
    if len(members) < _BOND_MIN_MEMBERS:
        return False
    previous = {}
    ...
    for a, b in pairs:
        ...
        partners[str(b["uid"])] = int(a["uid"])
        pair_rows.append({"a": int(a["uid"]), "an": a["name"], "b": int(b["uid"]), "bn": b["name"]})
    data = {"kind": "signal", "start": start.isoformat(), "end": end.isoformat(), "pairs": pair_rows, "partners": partners}
    await _rsetex(f"signal:{chat_id}", (_SIGNAL_HOURS + 1) * 3600, json.dumps(data, ensure_ascii=False))
    if bot and announce:
        try:
            names = "\n".join(f"{html.escape(x['an'])} × {html.escape(x['bn'])}" for x in pair_rows)
            await bot.send_message(
                chat_id,
                "◇ <b>SIGNAL SHIFT</b>\n\n"
                + names + "\n\n"
                + f"⏳ <b>{_remaining_text(end)}</b> until the next shift.",
                parse_mode="HTML"
            )
        except Exception as e:
            logger.info("Signal announcement skipped for %s: %s", chat_id, e)
    return True


async def _reveal_bond(chat_id, data, bot):
    rows = data.get("pairs", [])
    if not rows:
        return
    lines = [f"{html.escape(x['an'])} × {html.escape(x['bn'])}" for x in rows]
    text = (
        "✦ <b>MIDNIGHT BOND — REVEALED</b>\n\n" +
        "\n".join(lines) +
        "\n\n<i>The cycle is complete.</i>\n"
        f"Next bond · <b>{ORACLE_CYCLE_START_H:02d}:{ORACLE_CYCLE_START_M:02d}</b>"
    )
    await _rsetex(f"bond_reveal:{chat_id}", 3 * 86400, json.dumps(data, ensure_ascii=False))
    try:
        await bot.send_message(chat_id, text, parse_mode="HTML")
    except Exception as e:
        logger.info("Bond reveal skipped for %s: %s", chat_id, e)


async def _oracle_bond_scheduler(context: ContextTypes.DEFAULT_TYPE):
    """Restart-safe scheduler: derives state from clock, never from job survival."""
    now = datetime.now(ORACLE_TZ)
    bond_start, bond_end = _bond_cycle_bounds(now)
    signal_start, signal_end = _signal_cycle_bounds(now)
    for chat_id in list(_known_groups):
        try:
            raw = await _rget(f"bond:{chat_id}")
            current = json.loads(raw) if raw else None
            current_start = current.get("start") if current else None
            if current_start != bond_start.isoformat():
                # Reveal the previous cycle only once, then create today's bond.
                if current and current.get("start") != bond_start.isoformat():
                    marker = f"bond_revealed:{chat_id}:{current.get('start')}"
                    if not await _rexists(marker):
                        await _reveal_bond(chat_id, current, context.bot)
                        await _rsetex(marker, 3 * 86400, "1")
                await _make_bond(chat_id, bond_start, bond_end, announce=True, bot=context.bot)

            sraw = await _rget(f"signal:{chat_id}")
            signal = json.loads(sraw) if sraw else None
            if not signal or signal.get("start") != signal_start.isoformat():
                await _make_signal(chat_id, signal_start, signal_end, announce=bool(signal), bot=context.bot)
        except Exception:
            logger.exception("Automatic bond/signal cycle failed for chat %s", chat_id)


async def bond_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if update.effective_chat.type == "private":
        await update.message.reply_text("🌙 Bonds belong to a room. Use this inside your group.")
        return
    raw = await _rget(f"bond:{chat_id}")
    if not raw:
        await update.message.reply_text("✦ The next Midnight Bond is being prepared. Check again shortly.")
        return
    try:
        data = json.loads(raw)
        ...
        if not partner_uid:
            await update.message.reply_text("✦ You aren't in this cycle yet. Stay active and Midnight will notice.")
            return
        row = next((x for x in data.get("pairs", []) if x["a"] == uid or x["b"] == uid), None)
        partner = row["bn"] if row and row["a"] == uid else row["an"] if row else "someone"
        end = datetime.fromisoformat(data["end"])
        await update.message.reply_text(
            "✦ <b>MIDNIGHT BOND</b>\n\n"
            f"Your connection: <b>{html.escape(partner)}</b>\n\n"
            f"{ORACLE_CYCLE_START_H:02d}:{ORACLE_CYCLE_START_M:02d} → {ORACLE_CYCLE_START_H:02d}:{ORACLE_CYCLE_START_M:02d}\n⏳ <b>{_remaining_text(end)}</b> remaining",
            parse_mode="HTML"
        )
    except Exception:
        await update.message.reply_text("✦ Midnight is recalibrating this cycle. Try again in a moment.")


async def signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if update.effective_chat.type == "private":
        await update.message.reply_text("◇ The Signal belongs to a room. Use this inside your group.")
        return
    raw = await _rget(f"signal:{chat_id}")
    if not raw:
        await update.message.reply_text("◇ No active Signal yet. Midnight is setting the room.")
        return
    try:
        data = json.loads(raw)
        uid = update.effective_user.id
        ...
        await update.message.reply_text(
            ...
            f"⏳ <b>{_remaining_text(end)}</b> until the signal shifts.",
            parse_mode="HTML"
        )
    except Exception:
...
async def couples_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    raw = await _rget(f"bond_reveal:{chat_id}")
    if not raw:
        # Current cycle is intentionally hidden; reveal is stored separately at boundary.
        await update.message.reply_text(
            "✦ <b>TODAY'S BONDS</b>\n\nThe names are still behind the veil.\n\n"
            "Check back when Midnight reveals them.", parse_mode="HTML"
        )
        return
    try:
        data = json.loads(raw)
        lines = [f"{html.escape(x['an'])} × {html.escape(x['bn'])}" for x in data.get("pairs", [])]
        await update.message.reply_text(
            "✦ <b>TODAY'S BONDS</b>\n\n" + "\n".join(lines) + "\n\n<i>Revealed.</i>",
            parse_mode="HTML"
        )
...
async def bondstatus_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    raw = await _rget(f"bond:{chat_id}")
    if not raw:
        await update.message.reply_text("✦ No bond cycle is active yet.")
        return
    try:
        data = json.loads(raw)
        end = datetime.fromisoformat(data["end"])
        await update.message.reply_text(
            "✦ <b>MIDNIGHT BOND</b>\n\n"
            f"Cycle: <b>{ORACLE_CYCLE_START_H:02d}:{ORACLE_CYCLE_START_M:02d} → {ORACLE_CYCLE_START_H:02d}:{ORACLE_CYCLE_START_M:02d}</b>\n"
            f"Remaining: <b>{_remaining_text(end)}</b>\n"
            f"Pairs: <b>{len(data.get('pairs', []))}</b>",
            parse_mode="HTML"
        )
    except Exception:
...
# Legacy commands intentionally disabled: pairing is now automatic.
async def startcouple_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✦ Midnight chooses the bonds automatically now. No joining required.")

async def joincouple_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✦ There is nothing to join. Stay active — Midnight chooses automatically.")

async def couplestatus_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await bondstatus_command(update, context)

async def mymission_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🕯️ Missions are no longer part of the bond system. Midnight keeps this one clean.")
...
async def track_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _recent_members[chat_id] = existing[-100:]
    # Persist activity so automatic bonds survive Render restarts.
    try:
        key = f"oracle_active_members:{chat_id}"
        raw = await _rget(key)
        data = json.loads(raw) if raw else {}
        data[str(u.id)] = {"name": u.first_name or "Member", "last_seen": int(datetime.now(ORACLE_TZ).timestamp())}
        await _rsetex(key, (_BOND_ACTIVITY_DAYS + 2) * 86400, json.dumps(data, ensure_ascii=False))
    except Exception:
        logger.debug("Could not persist active member %s", u.id, exc_info=True)
...
def _cricket_card(title, body, footer="🏏 Midnight Cricket"):
...
async def cricket_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(_cricket_card("MIDNIGHT CRICKET", html.escape(reply)), parse_mode="HTML")
...
async def cricket_predict_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    body = (f"<b>{html.escape(team_a)}</b>  🆚  <b>{html.escape(team_b)}</b>\n\n"
            f"🔮 {html.escape(prediction)}\n\n"
            f"<b>Choose your side</b>\n"
            f"<code>/cbet {html.escape(team_a)} &lt;amount&gt;</code>\n"
            f"<code>/cbet {html.escape(team_b)} &lt;amount&gt;</code>")
    await update.message.reply_text(_cricket_card("ORACLE MATCH CALL", body, "The match decides the rest."), parse_mode="HTML")
...
async def cricket_play_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    body = (f"<b>{html.escape(p1_name)}</b>  {html.escape(p1_team)}  <code>{p1_score}</code>\n"
            f"<b>{html.escape(p2_name)}</b>  {html.escape(p2_team)}  <code>{p2_score}</code>\n\n"
            f"🏆 <b>{html.escape(winner_name)}</b> wins\n"
            f"🪙 <code>+200</code> coins\n\n"
            f"<i>{html.escape(random.choice(_CRICKET_DRAMA))}</i>")
    await update.message.reply_text(_cricket_card("MATCH RESULT", body), parse_mode="HTML")
...
BOT_COMMANDS=[
    ...
    BotCommand("signal",        "◇ your current rotating connection"),
    BotCommand("couples",       "✦ today's revealed bonds"),
    BotCommand("bondstatus",    "⏳ Midnight Bond cycle status"),
...
async def _post_init(app:Application):
    ...
    # Restart-safe automatic bond/signal engine. State is clock-derived + Redis-backed.
    app.job_queue.run_repeating(_oracle_bond_scheduler, interval=30, first=3, name="midnight_bond_scheduler")
    logger.info("✅ Automatic Midnight Bond + Signal scheduler armed")
