...
async def _get_bot_username(bot):
    ...
    if not _BOT_USERNAME:
        try: me = await bot.get_me(); _BOT_USERNAME = me.username
        except: _BOT_USERNAME = ""
    return _BOT_USERNAME

def _should_reply(message: Message, bot_username: str) -> bool:
    if message.chat.type == "private": return True
    text = (message.text or message.caption or "").lower()
    if bot_username and f"@{bot_username.lower()}" in text: return True
    if "midnight" in text: return True
...
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.send_chat_action(update.effective_chat.id, "typing")
    except: pass
    ...
    try: await message.reply_text(reply, parse_mode=ParseMode.MARKDOWN)
        ...
        try: await message.reply_text(reply.replace("*","").replace("_","").replace("`",""))
        except Exception as e: logger.error(f"AI reply failed: {e}")
...
# ══════════════════════════════════════════════════════════════════════════
BOT_COMMANDS=[
    BotCommand("start","Welcome message"),BotCommand("help","List all commands"),
    BotCommand("checkin","Daily check-in — earn coins + build streak"),
    BotCommand("streakcheck","View check-in streak"),
    BotCommand("vent","Post anonymously to the group"),
    BotCommand("coinboard","Coin richlist leaderboard"),
    BotCommand("daily","Claim daily coins"),BotCommand("balance","Check coin balance"),
    BotCommand("rob","Rob someone (reply)"),BotCommand("gamble","Gamble coins"),
    BotCommand("richest","Top coin holders"),BotCommand("leaderboard","Game rankings"),
    BotCommand("wallet","Your protected vault"),BotCommand("deposit","Lock coins in vault"),
    BotCommand("withdraw","Take coins from vault"),BotCommand("cgift","Gift coins (reply)"),
    BotCommand("setpass","Set backup password (DM only)"),BotCommand("recover","Recover account (DM only)"),
    BotCommand("bet","50/50 solo bet"),BotCommand("betstats","Betting stats"),
    BotCommand("topbet","Top bettors"),BotCommand("mines","Mines game"),
    BotCommand("oraclehour","Admin: trigger group event"),BotCommand("enter","Join active event"),
    BotCommand("eventcheck","Check active event"),
    BotCommand("oracle","Today's prophecy"),BotCommand("aura","Aura scan"),
    BotCommand("identity","Oracle identity card"),BotCommand("vibecheck","Full vibe check"),
    BotCommand("shadow","Your shadow self"),BotCommand("element","Cosmic element"),
    BotCommand("corecode","3 core personality words"),BotCommand("universe","Universe message"),
    BotCommand("ritual","Today's ritual"),BotCommand("duality","Light & dark side"),
    BotCommand("glitch","Oracle glitches"),BotCommand("nightreport","Night energy reading"),
    BotCommand("sigil","Personal sigil"),
    BotCommand("tarot","Draw a tarot card"),BotCommand("fate","Cryptic fate"),
    BotCommand("starsign","Zodiac reading"),BotCommand("dream","Dream interpretation"),
    BotCommand("manifest","Affirmation card"),BotCommand("confess","Anonymous confession"),
    BotCommand("quiz","Trivia round"),BotCommand("truth","Truth question"),
    BotCommand("dare","Dare challenge"),BotCommand("wyr","Would you rather"),
    BotCommand("nhie","Never have I ever"),BotCommand("rps","Rock paper scissors"),
    BotCommand("riddle","Get a riddle"),BotCommand("scramble","Unscramble a word"),
    BotCommand("guess","Guess a number"),BotCommand("dice","Roll dice"),
    BotCommand("slot","Spin slot machine"),BotCommand("hangman","Start hangman"),
    BotCommand("tictactoe","Challenge to tic-tac-toe"),BotCommand("wordchain","Word chain game"),
    BotCommand("trivia","Trivia by category"),BotCommand("wordle","Today's wordle"),
    BotCommand("impostor","Start impostor round"),
    BotCommand("hug","Hug someone"),BotCommand("pat","Pat someone"),
    BotCommand("slap","Slap someone"),BotCommand("kiss","Kiss someone"),
    BotCommand("ship","Ship two people"),BotCommand("roast","Playful roast"),
    BotCommand("compliment","Give a compliment"),BotCommand("8ball","Magic 8-ball"),
    BotCommand("quote","Random quote"),
    BotCommand("bestie","Declare a bestie"),BotCommand("duo","Duo name"),
    BotCommand("friendship","Compatibility score"),BotCommand("loyalty","Loyalty score"),
    BotCommand("crush","Pick a crush"),BotCommand("secretadmirer","Anonymous admirer DM"),
    BotCommand("mute","Mute user (admin)"),BotCommand("ban","Ban user (admin)"),
    BotCommand("kick","Kick user (admin)"),BotCommand("warn","Warn user (admin)"),
    BotCommand("warnings","Check warnings"),BotCommand("purge","Delete N messages (admin)"),
    BotCommand("rules","Show group rules"),BotCommand("setrules","Set rules (admin)"),
    BotCommand("id","Get user/chat ID"),BotCommand("info","User info card"),
    BotCommand("stats","Group activity stats"),BotCommand("topactive","Most active members"),
    BotCommand("afk","Mark yourself away"),BotCommand("report","Report a message"),
    BotCommand("poll","Create a poll"),BotCommand("timecapsule","Lock a message for later"),
    BotCommand("capsules","List time capsules"),
    BotCommand("marry","Propose to someone"),BotCommand("divorce","End a marriage"),
    BotCommand("profile","Your Oracle profile"),BotCommand("work","Earn coins by working"),
    BotCommand("shop","Browse item shop"),BotCommand("buy","Buy from shop"),
    BotCommand("inventory","Your inventory"),BotCommand("gift","Gift a shop item"),
    BotCommand("survive","Survive the death game"),BotCommand("deathgame","Start death game"),
    BotCommand("joingame","Join death game"),BotCommand("roulette","Russian roulette"),
]
...
def main():
    ...
    # New Aesthetic
    app.add_handler(CommandHandler("oracle",oracle_new_command))
    ...
    # Original Aesthetic (handlers/aesthetic.py)
