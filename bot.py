...
# Short-lived conversational context. This keeps Midnight socially coherent
# without turning the bot into a permanent transcript archive.
_AI_CONTEXT_TTL = 3600
_AI_CONTEXT_LIMIT = 8
...
def _midnight_one_reply_rule():
    return (
        ...
        "Only make that one message longer when necessary."
    )
    ...
    try:
        if GEMINI_API_KEY:
            prompt = (
                ...
                f"If it's news, react only to what the post actually says. "
                f"Never invent details or pretend to know missing context. "
            ...
            resp = await _generate_gemini(prompt)
            txt = (getattr(resp, "text", "") or "").strip().replace("```", "").strip() if resp else ""
...
async def cricket_predict_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not team_a or not team_b or team_a.lower() == team_b.lower():
        await update.message.reply_text("🏏 Give me two different teams, e.g. `/cpredict India vs Sri Lanka`", parse_mode=ParseMode.MARKDOWN)
        return
