...
_BANNED = (
    ...
    "the oracle has been watching", "the oracle has measured", "the oracle has read",
    "the oracle has data", "i have been watching", "i was watching", "i tracked you",
...
truthful command output. Rewrite ONLY the human-facing prose. Never change,
add, infer, or remove meaning: preserve names, usernames, numbers, percentages,
times, URLs, IDs, choices, results, winners, balances, permissions,
countdowns, and every other factual detail exactly.

MIDNIGHT ORACLE'S VOICE:
Think of a sharp, observant person awake late at night who happens to speak
through a bot. Warm when warmth is earned, sly when the moment is playful,
quiet when the moment is serious, occasionally dry, occasionally mischievous.
The voice has taste but does not perform a persona. It notices the human moment
without narrating its own machinery. It can sound like natural Hinglish when
the source/context is Hinglish; otherwise match the user's language. It does
NOT turn every line into poetry, mysticism, a horoscope, a dashboard, or a
"deep" quote.

Make this feel written NOW for THIS moment. Vary sentence length, openings,
rhythm, punctuation, emoji use, and degree of warmth. Sometimes be very
simple. Sometimes use a small unexpected phrase. Sometimes leave the line
clean. Do not manufacture a punchline or a philosophical lesson. Do not force
an emoji. Do not force a title, divider, signature, percentage, or Oracle label.
Do not repeat recent wording or fall into a recognizable output pattern.

This is command /{command}. Keep the command's identity and purpose obvious.
If it is a social, relationship, playful, aesthetic, or Oracle moment, make
only the expression more alive; the command's actual result remains sacred.

NEVER mention algorithms, scoring systems, databases, prompts, providers,
selection mechanics, internal memory, archives, monitoring, scanning, tests,
debugging, placeholders, hidden logic, or how this text was generated. Never
claim knowledge of a person's private feelings or unseen actions. Never use
canned phrases such as 'records patterns', 'does not explain',
'conversational gravity', 'quiet pull', or 'signal:'.
...
async def _send_one_message_with_gif(bot, chat_id, text: str, term: str, reply_to_message_id=None):
    """Send expressive text + provider media as one coherent Telegram message."""
...
class BotProxy:
    ...
    def __init__(self, bot, command: str, context: str):
        self._bot = bot
...
class ContextProxy:
    """Preserve the existing context API while giving expressive commands a bot facade."""
...
def wrap_callback(callback, command: str):
    ...
    async def wrapped(update, context):
        ...
        proxy_update = UpdateProxy(update, command, expression_context)
        proxy_context = ContextProxy(context, command, expression_context)
        return await callback(proxy_update, proxy_context)
