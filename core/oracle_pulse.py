...
async def pulse_callback(context) -> None:
    ...
    for group_id in targets:
        try:
            ...
            _log("ORACLE_PULSE_STAGE | stage=decision | chat=%s | speak=%s | strategy=%s", group_id, decision.speak, contract.strategy)
            ...
            if accepted is None:
                continue
            delivered = await _deliver_narrative_with_media(application, db, group_id, accepted.text, accepted.kind, None, now)
            if not delivered:
                continue
