...
class EconomyService:
    ...
    Group scope is encoded into the key. Every mutation is protected by a
    distributed lock; two-party transfers additionally use one Redis EVAL so a
    process crash cannot leave one account debited without crediting the other.
    """
    ...
    async def remove(self, user_id: int, amount: int, reason: str = "spend", scope: int | str | None = None) -> Transaction:
        ...
        async with self.store.lock(f"economy:{scope}:{int(user_id)}") as acquired:
            ...
            try:
                ...
                raise EconomyError("persistent economy is temporarily unavailable") from exc
            if new_balance < 0:
                # This should be impossible while our lock is held. Do not hide
                # corruption if a backend behaves unexpectedly.
        ...
        async with self.store.lock(f"economy:{scope}:{first}") as first_lock:
            ...
            async with self.store.lock(f"economy:{scope}:{second}") as second_lock:
                if not second_lock:
                    raise EconomyError("economy is busy; please retry")
                try:
                    sender_balance, receiver_balance = await self.store.atomic_transfer(
                        self.key(sender, scope), self.key(receiver, scope), amount
                    )
                except StorageError as exc:
                    ...
                    raise EconomyError("persistent economy is temporarily unavailable") from exc
                return (
                    Transaction(sender, -amount, sender_balance, reason),
