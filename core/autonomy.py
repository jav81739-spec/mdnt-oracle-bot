...
async def record_activity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not isinstance(members, dict):
        members = {}
    members[str(user.id)] = {"user_id": int(user.id), "name": user.first_name or "Midnight Soul", "username": user.username, "seen": time.time()}
...
async def _event(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if len(people) < 2:
        return False
    kinds = ["choice", "duo", "spotlight", "oracle_question", "duel", "midnight", "fate_card", "mystery_pair", "trio", "vibe_check", "hidden_mvp", "chain_reaction"]
    weights = [22, 17, 14, 13, 8, 5, 5, 4, 3, 3, 3, 3]
    kind = random.choices(kinds, weights=weights, k=1)[0]
    a, b = random.sample(people, 2)
    A, B = _render_mention(a), _render_mention(b)
    if kind == "choice":
        text = f"<b>✦ 𝐓𝐇𝐄 𝐎𝐑𝐀𝐂𝐋𝐄 𝐇𝐀𝐒 𝐂𝐇𝐎𝐒𝐄𝐍 ✦</b>\n\n{A} × {B}\n\n<i>No nominations. No applications. The night simply picked you two.</i> 🌙"
    elif kind == "duo":
        text = f"<b>☾ 𝐌𝐈𝐃𝐍𝐈𝐆𝐇𝐓 𝐃𝐔𝐎 ☽</b>\n\nTonight's unexpected pair: {A} + {B}\n\n<i>One conversation. One tiny mission. Make it interesting.</i> ✦"
    elif kind == "spotlight":
        T = _render_mention(random.choice(people)); text = f"<b>⟡ 𝐓𝐎𝐍𝐈𝐆𝐇𝐓'𝐒 𝐒𝐏𝐎𝐓𝐋𝐈𝐆𝐇𝐓 ⟡</b>\n\n{T}\n\n<i>The Oracle noticed you. That's all you're getting for now.</i> 🌘"
    elif kind == "oracle_question":
        T = _render_mention(random.choice(people)); question = random.choice(["What is one thing you would change about tonight?", "Who in this group has the most chaotic energy?", "What song belongs to this exact moment?", "Describe your current vibe in exactly three words."])
        text = f"<b>☽ 𝐎𝐑𝐀𝐂𝐋𝐄 𝐏𝐈𝐂𝐊𝐒 𝐀 𝐐𝐔𝐄𝐒𝐓𝐈𝐎𝐍 ☾</b>\n\n{T}, {question}\n\n<i>No pressure. Just answer if you feel like it.</i>"
    elif kind == "duel":
        text = f"<b>⚔️ 𝐓𝐖𝐎 𝐍𝐀𝐌𝐄𝐒. 𝐎𝐍𝐄 𝐑𝐎𝐔𝐍𝐃.</b>\n\n{A} vs {B}\n\n<i>The group decides the category. The Oracle refuses to explain why.</i> 🌙"
    elif kind == "fate_card":
        T = _render_mention(random.choice(people)); title, line = random.choice([("THE SPARK", "You are more influential in this chat than you realise."), ("THE MOON", "Stay quiet tonight. Something interesting may come to you."), ("THE MIRROR", "Someone here probably matches your current energy."), ("THE COMET", "Do something unexpected before the night ends.")])
        text = f"<b>𖤓 𝐅𝐀𝐓𝐄 𝐂𝐀𝐑𝐃 · {title} 𖤓</b>\n\n{T}\n<i>{line}</i> ✦"
    elif kind == "mystery_pair":
        text = f"<b>◈ 𝐌𝐘𝐒𝐓𝐄𝐑𝐘 𝐏𝐀𝐈𝐑 ◈</b>\n\n{A} ↔ {B}\n\n<i>The Oracle has a reason. It isn't telling the group yet.</i> 👁️"
    elif kind == "trio":
        candidates = [p for p in people if p.user_id not in {a.user_id, b.user_id}]
        if candidates:
            C = _render_mention(random.choice(candidates)); text = f"<b>✧ 𝐓𝐇𝐄 𝐓𝐑𝐈𝐀𝐃 ✧</b>\n\n{A} · {B} · {C}\n\n<i>Three completely unrelated people. One suspiciously good combination.</i> 🌙"
        else:
            text = f"<b>✧ 𝐓𝐇𝐄 𝐓𝐑𝐈𝐀𝐃 ✧</b>\n\n{A} · {B}\n\n<i>The Oracle is waiting for a third soul.</i>"
    elif kind == "vibe_check":
        T = _render_mention(random.choice(people)); vibe = random.choice(["main-character energy", "quietly chaotic", "dangerously unbothered", "soft but plotting", "one message away from causing lore"])
        text = f"<b>☾ 𝐕𝐈𝐁𝐄 𝐂𝐇𝐄𝐂𝐊 ☽</b>\n\n{T}\n<i>Current reading: {vibe}.</i> 🖤"
    elif kind == "hidden_mvp":
        T = _render_mention(max(people, key=lambda p: p.seen)); text = f"<b>🏆 𝐇𝐈𝐃𝐃𝐄𝐍 𝐌𝐕𝐏</b>\n\n{T}\n\n<i>The Oracle has been watching the activity. Quietly carrying the room.</i> ✦"
    elif kind == "chain_reaction":
        text = f"<b>⚡ 𝐂𝐇𝐀𝐈𝐍 𝐑𝐄𝐀𝐂𝐓𝐈𝐎𝐍 ⚡</b>\n\n{A} starts it. {B} decides where it goes.\n\n<i>Reply with one word that describes tonight.</i> 🌙"
    else:
        text = f"<b>𖤓 𝐀𝐅𝐓𝐄𝐑 𝐌𝐈𝐃𝐍𝐈𝐆𝐇𝐓 𖤓</b>\n\n{A} and {B} have been quietly selected.\n\n<i>Something about this pairing felt interesting.</i> 🖤"
...
async def autonomous_tick(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for key in keys[:30]:
        try:
            ...
            continue
        if await _already_busy(chat_id) or random.random() > 0.18:
            continue
        try:
