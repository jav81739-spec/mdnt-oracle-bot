...
_LEASE_KEY="midnight:polling_lease";_LEASE_TTL=60;_LEASE_REFRESH=20;_LEASE_WAIT_MAX=90;_INSTANCE_ID=f"{socket.gethostname()}:{os.getpid()}";_storage=None;_app=None;_lease_task:Optional[asyncio.Task]=None;_health_server:Optional[HTTPServer]=None;_shutting_down=False;_ready=False
...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/","/health","/healthz"):
            ...
            ready=_ready and (not _shutting_down);self._respond(200 if ready else 503,json.dumps({"ready":ready}).encode(),"application/json")
...
async def _graceful_shutdown():
    ...
    _shutting_down=True;_ready=False;log.info("Graceful shutdown started")
    current=asyncio.current_task()
    if _lease_task and not _lease_task.done() and _lease_task is not current:
        _lease_task.cancel()
...
async def run(application,storage_client=None):
    ...
    _storage=storage_client;_app=application;_shutting_down=False;_ready=False;loop=asyncio.get_running_loop();_install_signal_handlers(loop);start_health_server()
    ...
    try:
        ...
        await application.updater.start_polling(drop_pending_updates=True,allowed_updates=["message","edited_message","callback_query","chat_member","my_chat_member","poll_answer","poll","inline_query"]);_ready=True;log.info("READINESS=ready | telegram_polling=active")
