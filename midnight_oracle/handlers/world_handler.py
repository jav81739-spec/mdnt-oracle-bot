...
async def start_game(update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
    ...
    if name=='wyr':
        try:
            ...
            if sched:
                sched.scheduler.add_job(WouldYouRatherGame(db).close_poll,'date',run_date=poll.date if getattr(poll,'date',None) else __import__('datetime').datetime.now(sched.timezone)+__import__('datetime').timedelta(seconds=60),args=[poll.poll.id,context.bot,update.effective_chat.id],id=f'wyr_close_{poll.poll.id}',replace_existing=True)
        except Exception as exc:await update.effective_message.reply_text('☾ Could not open that poll right now.')
        return
...
def _schedule_scramble(context:ContextTypes.DEFAULT_TYPE,group_id:int)->None:
    ...
    if sched and db:
        sched.scheduler.add_job(_scramble_timeout,'date',run_date=__import__('datetime').datetime.now(sched.timezone)+__import__('datetime').timedelta(seconds=30),args=[context.application,db,group_id],id=f'scramble_timeout_{group_id}',replace_existing=True)
async def _scramble_timeout(application,db,group_id:int)->None:
    ...
    if text:
        ...
        if row and json.loads(row['state']).get('awaiting_answer'):
            ...
            if sched:sched.scheduler.add_job(_scramble_timeout,'date',run_date=__import__('datetime').datetime.now(sched.timezone)+__import__('datetime').timedelta(seconds=30),args=[application,db,group_id],id=f'scramble_timeout_{group_id}',replace_existing=True)
        elif row is None: return
...
async def handle_game_message(update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
    ...
    if finished:
        ...
        if next_text:
            await msg.reply_text(next_text);_schedule_scramble(context,chat.id)
...
async def handle_poll_answer(update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
    ...
    if db and p.option_ids:await WouldYouRatherGame(db).handle_poll_answer(p.poll_id,p.user.id,p.option_ids[0])
...
async def handle_poll(update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
    ...
    if not db:return
    rows=await db.fetchall("SELECT state FROM game_sessions WHERE game_type='would_you_rather' AND is_active=1")
    for r in rows:
...
async def end_game(update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
    """Gracefully end an active game and cancel its timeout."""
    if not update.effective_chat:return
    db=context.application.bot_data['oracle_db'];gid=update.effective_chat.id;row=await db.fetchone('SELECT game_type FROM game_sessions WHERE group_id=? AND is_active=1 ORDER BY id DESC LIMIT 1',(gid,));sched=context.application.bot_data.get('oracle_scheduler')
    if row and sched:
        try:sched.scheduler.remove_job(f'scramble_timeout_{gid}')
        except Exception:pass
        if row['game_type']=='would_you_rather':
            # Native polls can be stopped by Telegram; persistence is closed by the engine.
            pass
