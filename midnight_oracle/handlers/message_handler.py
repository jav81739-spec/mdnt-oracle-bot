...
class MessageRouter:
    ...
    async def handle(self,update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
        ...
        try:
            ...
            await db.upsert_member(user.id,chat.id,user.username or '',user.first_name or 'friend');profile=await self.memory.get(user.id,chat.id)
            recent=await load_recent(storage_client,str(chat.id));self.recent[chat.id]=recent;now=datetime.now();signal=self.mood.estimate(text)
            ctx=GroupContext(str(user.id),str(chat.id),list(recent)[-10:],now.hour,now.hour>=23 or now.hour<3,chat.title or '',profile.relationship_tier,profile.preferred_name or user.first_name or 'friend',now_ts(),(' | '.join(list(profile.themes[:2])+list(profile.worries[:1]))) or 'none')
            if self.jokes:
                await self.jokes.observe(text,user.id,chat.id);callback=await self.jokes.detect_callback_opportunity(text,chat.id)
                if callback and not await db.cooldown_active('group',str(chat.id),'ambient'):
                    await message.reply_text(callback);await self.memory.observe(user.id,chat.id,ctx.sender_name,text,True);recent.append(text);del recent[:-8];await save_recent(storage_client,str(chat.id),recent);await self._announce_achievements(message,user,chat.id,'oracle_reply');return
            if self.identity:await self.identity.update(chat.id,text,signal)
            if direct:
                reply=await self.replies.generate(chat.title or 'Midnight Oracle',ctx.sender_name,ctx.relationship_tier,text,signal.summary(),str(ctx.hour),ctx.is_late_night,ctx.memory_snippet);await message.reply_text(reply);await self.memory.observe(user.id,chat.id,ctx.sender_name,text,True);recent.append(text);del recent[:-8];await save_recent(storage_client,str(chat.id),recent);await self._announce_achievements(message,user,chat.id,'oracle_reply');return
            if self.stickers:
                ...
                if media.should_send:
                    ...
                    await self.stickers.record(chat.id,'contextual',media.sticker_id);await self.memory.observe(user.id,chat.id,ctx.sender_name,text,True);recent.append(text);del recent[:-8];await save_recent(storage_client,str(chat.id),recent);return
            decision=await self.engine.process_message(message,ctx);await self.memory.observe(user.id,chat.id,ctx.sender_name,text,decision.should_reply or signal.social>=.5);recent.append(text);del recent[:-8];await save_recent(storage_client,str(chat.id),recent)
            if decision.should_reply and decision.reply_text:
                await message.reply_text(decision.reply_text);await self._announce_achievements(message,user,chat.id,'oracle_reply')
        except Exception as exc:
            await soft_alert(context.application.bot_data.get('storage_client') if context and context.application else None,'message_router',exc)
    @staticmethod
