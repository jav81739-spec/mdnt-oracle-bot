...
class MessageRouter:
    ...
    def __init__(self,engine:FriendEngine,memory:MemoryEngine,mood:MoodEngine,replies:ReplyGenerator|None=None)->None:
        """Bind all social engines to the Phase 1 router.""";self.engine=engine;self.memory=memory;self.mood=mood;self.replies=replies or ReplyGenerator();self.recent={};self.jokes=JokeEngine(engine.db);self.identity=GroupIdentityEngine(engine.db);self.stickers=StickerHandler(engine.db)
    async def handle(self,update:Update,context:ContextTypes.DEFAULT_TYPE)->None:
        ...
        try:
            ...
            await self.engine.db.execute("INSERT INTO group_profile(group_id,group_name,timezone,created_at) VALUES(?,?,?,?) ON CONFLICT(group_id) DO UPDATE SET group_name=excluded.group_name",(chat.id,chat.title or '','Asia/Kolkata',now_ts()))
            await self.engine.db.upsert_member(user.id,chat.id,user.username or '',user.first_name or 'friend')
            profile=await self.memory.get(user.id,chat.id); recent=self.recent.setdefault(chat.id,[]); now=datetime.now(); signal=self.mood.estimate(text)
            ...
            if decision.should_reply and decision.reply_text:
                await message.reply_text(decision.reply_text);return
            ...
            if media.should_send:
                ...
                await self.stickers.record(chat.id,'contextual',media.sticker_id)
