...
CREATE TABLE IF NOT EXISTS oracle_memory (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,group_id INTEGER NOT NULL,memory_type TEXT NOT NULL,content TEXT NOT NULL,importance REAL DEFAULT 0.5,created_at REAL NOT NULL,last_used_at REAL DEFAULT 0);
CREATE INDEX IF NOT EXISTS idx_oracle_memory_lookup ON oracle_memory(user_id,group_id,importance,last_used_at);
...
CREATE TABLE IF NOT EXISTS predictions (id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER NOT NULL,predictor_user_id INTEGER NOT NULL,prediction_text TEXT NOT NULL,reveal_date REAL,actual_outcome TEXT DEFAULT '',created_at REAL NOT NULL);
...
class Database:
    ...
    async def update_wyr_votes(self,poll_id:str,user_id:int,option:int)->bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                (a if option==0 else b).append(uid);await self.db.execute('UPDATE game_sessions SET state=? WHERE id=? AND is_active=1',(json.dumps(state),sid));await self.db.commit();return True
    ...
    async def mark_revealed(self,event_id:int,revealed_by:int|None,message_id:int|None)->bool:
        async with self._lock:
            cur=await self.db.execute('UPDATE secret_events_log SET revealed_at=?,revealed_by=?,teaser_message_id=COALESCE(teaser_message_id,?) WHERE id=? AND revealed_at IS NULL',(str(now_ts()),revealed_by,message_id,event_id));await self.db.commit();return cur.rowcount==1