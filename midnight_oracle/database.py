...
CREATE TABLE IF NOT EXISTS secret_events_log (id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER NOT NULL,event_type TEXT NOT NULL,content TEXT NOT NULL,sent_at REAL NOT NULL,revealed_at TEXT DEFAULT NULL,revealed_by INTEGER DEFAULT NULL,teaser_message_id INTEGER DEFAULT NULL);
"""
...
class Database:
    ...
    async def connect(self) -> None:
        ...
        # so WAL pragma is safe and schema is fully committed after this.
        await self._db.executescript(SCHEMA)
        # Schema is now committed — safe to inspect columns.
        cols = await self._table_columns('secret_events_log')
        for name, definition in [
            ('revealed_at', 'TEXT DEFAULT NULL'),
            ('revealed_by', 'INTEGER DEFAULT NULL'),
            ('teaser_message_id', 'INTEGER DEFAULT NULL'),
        ]:
            if name not in cols:
                await self._db.execute(
                    f'ALTER TABLE secret_events_log ADD COLUMN {name} {definition}'
                )
        await self._db.commit()

    async def _table_columns(self, table: str) -> set[str]:
        """Return existing column names for a table."""
        # Use self._db directly — avoids the guard check during connect()
    ...
    async def upsert_member(self, user_id: int, group_id: int, username: str, name: str) -> None:
        """Create or refresh a member without replacing learned fields."""
        ts = now_ts()
        await self.execute(
            "INSERT INTO members(user_id,group_id,username,preferred_name,last_seen,created_at) "
            "VALUES(?,?,?,?,?,?) ON CONFLICT(user_id,group_id) DO UPDATE SET "
            "username=excluded.username,last_seen=excluded.last_seen,"
            "preferred_name=CASE WHEN members.preferred_name='' THEN excluded.preferred_name "
            "ELSE members.preferred_name END",
            (user_id, group_id, username or '', name or '', ts, ts),
        )

    async def increment_interaction(self, user_id: int, group_id: int) -> None:
        """Increment member interaction count and last-seen time."""
        await self.execute(
            'UPDATE members SET interaction_count=interaction_count+1,last_seen=? WHERE user_id=? AND group_id=?',
            (now_ts(), user_id, group_id),
        )

    async def add_memory(self, user_id: int, group_id: int, memory_type: str, content: str) -> None:
        """Store bounded member memory."""
        await self.execute(
            'INSERT INTO member_memory(user_id,group_id,memory_type,content,created_at) VALUES(?,?,?,?,?)',
            (user_id, group_id, memory_type, content[:500], now_ts()),
        )
        await self.execute(
            'UPDATE member_memory SET is_active=0 WHERE id IN '
            '(SELECT id FROM member_memory WHERE user_id=? AND group_id=? AND memory_type=? '
            'AND is_active=1 ORDER BY created_at DESC LIMIT -1 OFFSET 10)',
            (user_id, group_id, memory_type),
        )

    async def memories(self, user_id: int, group_id: int, memory_type: str | None = None, limit: int = 10) -> list[str]:
        """Return recent active memories."""
        q = 'SELECT content FROM member_memory WHERE user_id=? AND group_id=? AND is_active=1'
        p: list = [user_id, group_id]
        if memory_type:
            q += ' AND memory_type=?'
            p.append(memory_type)
        q += ' ORDER BY created_at DESC LIMIT ?'
        p.append(limit)
        rows = await self.fetchall(q, tuple(p))
        return [str(r[0]) for r in rows]

    async def delete_memories_matching(self, user_id: int, group_id: int, term: str) -> int:
        """Deactivate memories containing a requested term."""
        cur = await self.db.execute(
            'UPDATE member_memory SET is_active=0 WHERE user_id=? AND group_id=? AND is_active=1 AND content LIKE ?',
            (user_id, group_id, f'%{term}%'),
        )
        await self.db.commit()
        return cur.rowcount
    ...
    async def set_cooldown(self, scope: str, scope_id: str, kind: str, expires_at: float) -> None:
        ...
        await self.execute(
            'INSERT INTO cooldowns(scope,scope_id,cooldown_type,expires_at) VALUES(?,?,?,?) '
            'ON CONFLICT(scope,scope_id,cooldown_type) DO UPDATE SET expires_at=excluded.expires_at',
            (scope, scope_id, kind, expires_at),
        )
    ...
    async def cooldown_active(self, scope: str, scope_id: str, kind: str, at: float | None = None) -> bool:
        ...
        r = await self.fetchone(
            'SELECT expires_at FROM cooldowns WHERE scope=? AND scope_id=? AND cooldown_type=?',
            (scope, scope_id, kind),
        )
        return bool(r and float(r[0]) > (at or now_ts()))
    ...
    async def get_active_wyr_session(self, group_id: int) -> aiosqlite.Row | None:
        """Return the active WYR session for a group."""
        return await self.fetchone(
            "SELECT * FROM game_sessions WHERE group_id=? AND game_type='would_you_rather' AND is_active=1 ORDER BY id DESC LIMIT 1",
            (group_id,),
        )
    ...
    async def update_wyr_votes(self, poll_id: str, user_id: int, option: int) -> bool:
        ...
        try:
            async with self.db.execute(
                "SELECT id,state FROM game_sessions WHERE game_type='would_you_rather' AND is_active=1"
            ) as cur:
                rows = await cur.fetchall()
            import json
            found = None
            for r in rows:
                try:
                    ...
                    continue
                if str(s.get('poll_id')) == str(poll_id):
                    found = (int(r[0]), s)
                    break
            if not found:
            ...
            sid, state = found
            uid = str(user_id)
            a = state.setdefault('voters_a', [])
            b = state.setdefault('voters_b', [])
            if uid in a:
                a.remove(uid)
            if uid in b:
                b.remove(uid)
            ...
            await self.db.execute(
                'UPDATE game_sessions SET state=? WHERE id=? AND is_active=1',
    ...
    async def close_wyr_session(self, poll_id: str, result: dict) -> bool:
        ...
        try:
            async with self.db.execute(
                "SELECT id,group_id,state FROM game_sessions WHERE game_type='would_you_rather' AND is_active=1"
            ) as cur:
                rows = await cur.fetchall()
            row = None
            for r in rows:
                try:
                    if str(json.loads(r[2]).get('poll_id')) == str(poll_id):
                        row = r
                        break
                except (TypeError, ValueError):
                    continue
            if not row:
                await self.db.rollback()
                return False
            sid, gid = int(row[0]), int(row[1])
            cur = await self.db.execute(
                'UPDATE game_sessions SET is_active=0,ended_at=? WHERE id=? AND is_active=1',
                (now_ts(), sid),
            )
            if cur.rowcount != 1:
                await self.db.rollback()
                return False
            await self.db.execute(
                'INSERT INTO game_history(group_id,game_type,winner_user_id,summary,played_at) VALUES(?,?,?,?,?)',
                (gid, 'would_you_rather', None, json.dumps(result), now_ts()),
            )
            await self.db.commit()
            return True
        except Exception:
    ...
    async def is_revealed(self, event_id: int) -> bool:
        """Return whether a secret event has already been revealed."""
        r = await self.fetchone('SELECT revealed_at FROM secret_events_log WHERE id=?', (event_id,))
        return bool(r and r[0])

    async def mark_revealed(self, event_id: int, revealed_by: int | None, message_id: int | None) -> bool:
        """Mark an event revealed using compare-and-set semantics."""
        cur = await self.db.execute(
            'UPDATE secret_events_log SET revealed_at=?,revealed_by=?,teaser_message_id=COALESCE(teaser_message_id,?) '
            'WHERE id=? AND revealed_at IS NULL',
            (str(now_ts()), revealed_by, message_id, event_id),
        )
        await self.db.commit()
