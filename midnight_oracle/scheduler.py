...
class OracleScheduler:
    ...
    async def secret_daily(self)->None:
        ...
        for r in await self.db.fetchall('SELECT group_id FROM group_profile'):
            ...
            try:
                msg=await self.application.bot.send_message(event.group_id,teaser);eid=await engine.record(event,msg.message_id);await self.application.bot.edit_message_reply_markup(event.group_id,msg.message_id,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Reveal 👁',callback_data=f'reveal_{eid}')]]));self.scheduler.add_job(engine.reveal,'date',run_date=datetime.now(self.timezone)+timedelta(minutes=30),args=[eid,self.application.bot,msg.message_id,None,event.group_id],id=f'auto_reveal_{eid}',replace_existing=True)
            except Exception:continue
