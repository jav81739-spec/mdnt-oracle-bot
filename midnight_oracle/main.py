...
async def _post_init(application:Application)->None:
    ...
    db=Database(DATABASE_PATH); await db.connect(); mood=MoodEngine(); mem=MemoryEngine(db); engine=FriendEngine(db,mood); router=MessageRouter(engine,mem,mood)
    application.bot_data.update(oracle_db=db,oracle_router=router); scheduler=OracleScheduler(application,db); scheduler.start(); application.bot_data['oracle_scheduler']=scheduler
    log.info('AUTONOMOUS_CANONICAL_READY | friend_engine=on | memory=on | scheduler=on | world=on')
...
def build_application()->Application:
    ...
    commands={'start':start,'help':help_command,'oracle':oracle,'truth':truth,'memory':memory,'mymemory':mymemory,'forget':forget,'quiet':quiet,'wake':wake,'house':house,'tod':start_game,'wyr':start_game,'nhie':start_game,'scramble':start_game,'endgame':end_game}
