...
class SecretEventEngine:
    ...
    async def evaluate(self,group_id:int)->SecretEvent|None:
        ...
        r=await self.db.fetchone('SELECT COUNT(*) FROM secret_events_log WHERE group_id=? AND sent_at>?',(group_id,now_ts()-604800))
        if r and int(r[0])>=2:return None
        r=await self.db.fetchone('SELECT COUNT(*) FROM mood_log WHERE group_id=? AND timestamp>?',(group_id,now_ts()-86400))
        if not r or int(r[0])<8:return None
        return SecretEvent(group_id,'activity_stat',f'Oracle noticed {int(r[0])} mood signals in this room today. The room has been busy.')
    async def format_event(self,event:SecretEvent)->tuple[str,str]:
        """Return teaser and full reveal text.""";return '☾ Oracle has noticed something.','☾ Oracle noticed:\n\n'+event.content+'\n\n— observed quietly 🌙'
    async def record(self,event:SecretEvent,message_id:int|None=None)->int:
        """Persist a sent event and return its id.""";cur=await self.db.db.execute('INSERT INTO secret_events_log(group_id,event_type,content,sent_at,teaser_message_id) VALUES(?,?,?,?,?)',(event.group_id,event.event_type,event.content,now_ts(),message_id));await self.db.db.commit();return int(cur.lastrowid)
    ...
    async def recover_unrevealed(self,bot)->None:
        ...
        for r in rows:await self.reveal(int(r['id']),bot,int(r['teaser_message_id'] or 0),None,int(r['group_id']))
